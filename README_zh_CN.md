...
7. 假如你的图表已经展示了数据，但现在需要重新发请求，并展示一个跟原来不一样的图表（从柱状图变为饼图），你可以在请求前调用一下 reInit 方法，该方法会自动重置图表，并回到一开始的加载状态。但如果你只是需要重新请求数据，并不改变图表类型时，那你只需把请求数据前的 reInit 方法改成 setLoading 即可
